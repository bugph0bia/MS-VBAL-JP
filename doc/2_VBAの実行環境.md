...
すべてのVBAプログラムコードは、プロジェクト（セクション 4.1）の一部である。VBA 環境には、1 つ以上の名前の付いたプロジェクトを含めることができる。プロジェクトは、実装定義されたメカニズムを使用して VBA 環境に作成され読み込まれる。さらに、VBA 環境にはプロジェクトを変更または削除するための実装メカニズムを<ins
>含めてもよい</ins>。

## 2.7 拡張環境

VBA プロジェクト（セクション 4.1）内で VBA ソースコードを使用して定義されたエンティティ（セクション 2.2）に加えて、VBA 環境には他のソース内および他のメカニズムを使用して定義されたエンティティを含めることができる。VBA プログラムコードからアクセスすると、そのような外部環境エンティティは VBA 言語を使用して実装された環境エンティティであるかのように表示され動作する。

### 2.7.1 VBA 標準ライブラリ

VBA 標準ライブラリ（セクション 6）は、すべての VBA 環境に存在しなければならないエンティティ（セクション 2.2）の集合である。

これらのエンティティを VBA 言語コードから参照できるようにするために明示的な操作は必要ない。

### 2.7.2 外部の変数、プロシージャ、オブジェクト

VBA 環境には、VBA プログラミング言語を使用して明示的に定義されたエンティティ（セクション 2.2）の他に、他のプログラミング言語を使用して定義されたエンティティを含めることができる。VBA 言語の観点からは、このようなエンティティは、特性や性質が実装定義である外部ライブラリによって定義されたものとみなされる。

### 2.7.3 ホスト 環境

ホストアプリケーションは、実装依存のメカニズムを使用して、ホストされた VBA 環境内でアクセス可能な追加のエンティティ（セクション 2.2）を定義することができる。このようなエンティティは、VBA の実装とホストアプリケーションによって VBA 標準ライブラリ（セクション 2.7.1）と同様に直接アクセスすることができたり、外部ライブラリや定義済み VBA プロジェクト（セクション 2.6）として表示されたりすることがある。

VBA 実装と連携するホストアプリケーションは、VBA のファイル I/O モデルを、アプリケーション固有もしくはプラットフォームが持つファイルストレージ機構にマッピングする役割も担っている。

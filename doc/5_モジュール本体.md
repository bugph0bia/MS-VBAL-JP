...
- \<external-sub\> 要素は関数宣言であり、\<external-function\> はサブルーチン宣言である。
...
- \<event-parameter-list\> に含まれる \<positional-param\> 要素は、いかなる変数や変数バインディングも定義しない。それらは単に、関連付けられたイベント名を参照する \<raiseevent-statement\> に提供されなければならない引数を記述するだけである。

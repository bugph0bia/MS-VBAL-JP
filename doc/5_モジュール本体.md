...
- \<exit-for-statement\> は構文上 \<for-statement\> または \<for-each-statement\> の中に<ins>含まれなければならない<ins/>。

実行時セマンティクス

- このステートメントを囲む \<for-statement\> または \<for-each-statement\> のうち、最も字句の近いものの実行が直ちに完了する。\<exit-for-statement\> に続く \<statement-block\> 内の他のステートメントは実行されない。

#### 5.4.2.6 Do ステートメント

\<do-statement\> は、指定された事前/事後条件が True である限り一連のステートメントを実行する。
...
- キーワード `Do` またはキーワード `Loop` の後に指定できる \<condition-clause\> は一方のみで両方を指定することはできない。\<until-clause\> が指定された場合、\<boolean-expression\> の値を "Not (\<boolean-expression\>)" とした \<while-clause\> と同じ効果がある。
- キーワード `Do` またはキーワード `Loop` の後に \<condition-clause\> を指定しない場合、`Do` の後に、 "True" を持つ \<while-clause\> を含んでいる \<condition-clause\> を指定した場合と同様の効果がある。

実行時セマンティクス

- \<do-statement\> は、\<condition-clause\> を繰り返し評価し、もし True と評価されれば \<statement-block\> を実行する。\<condition-clause\> の評価と \<statement-block\> の実行順序は以下の表で定義される。

| \<condition-clause\> の位置 | 結果 |
|----|----|
| なし | ループの実行は、\<exit-do-statement\> が実行されるまで継続される。 |
| "Do" の直後 | \<statement-block\> を実行する前に \<condition-clause\> が評価される。もし False と評価されれば \<statement-clause\> が実行される。\<br/\>\<br/\>\<statement-block\> と現在のステートメントの実行は直ちに終了する。 |
| "Loop" の直後 | \<statement-block\> は \<condition-clause\> が評価される前に実行される。もし True と評価されれば、\<statement-block\> が再び実行されてこの処理が繰り返される。\<br/\>\<br/\>もし False と評価されれば、\<statement-block\> と現在のステートメントの実行は直ちに終了する。 |

#### 5.4.2.7 Exit Do ステートメント
...
- \<exit-do-statement\> は構文上 \<do-statement\> の中に<ins>含まれなければならない</ins>。

実行時セマンティクス

- \<statement-block\> の中で、構文上で最も近くにある \<do-statement\> を含む \<exit-do-statement\> を実行した場合、\<statement-block\> とこのステートメントの実行は直ちに終了する。\<statement-block\> 内の \<exit-do-statement\> に続く他のステートメントは実行されない。

#### 5.4.2.8 If ステートメント

\<if-statement\> は、\<statement-block\> を実行するか否かを決定する。
...
- \<if-statement\> は \<boolean-expression\> を評価し、もし True と評価されれば "Then" 以降の \<statement-block\> を実行する。False と評価された場合は、以下の順序で実行される。
    1. 各 \<else-if-block\> の \<boolean-expression\>     を順に、True を見つけるまで評価する。その \<else-if-block\> を含む \<statement-block\> が実行され、\<if-statement\> の実行を終了する。
    2. \<else-if-block\> の \<boolean-expression\> がいずれも True と評価されず、かつ \<else-block\> が存在する場合、\<else-block\> の \<statement-block\> が実行される。
- \<if-statement\> の外で定義された \<goto-statement\> が \<statement-block\> 内の \<statement\> を実行させる場合、\<boolean-expression\> は評価されない。また、\<goto-statement\> によって \<statement-block\> から抜け出すこともできる。後の \<goto-statement\> によって \<statement-block\> に再び入った場合の動作は、\<if-statement\> を実行する際のルールと同じである。

#### 5.4.2.9 単一行 If ステートメント

- \<single-line-if-statement\>は、ステートメントを実行するか否かを決定する。
...
- \<single-line-if-statement\>は、`Then` キーワードの直後に \<list-or-label\> または \<single-line-else-clause\> が存在することによって \<if-statement\> と区別される。
- \<single-line-if-statement\> は、\<same-line-statement\> の全体を含む 1 つの論理行で<ins>定義されなければらない</ins>。この制限により、\<LINE-END\> 要素を必要とする埋め込み型の \<EOS\> 選択肢は排除される。
- \<single-line-if-statement\> の \<list-or-label\> が \<single-line-if-statement\> を含むとき、最初の\ <single-line-else-clause\> は直前の \<single-line-if-statement\> に含まれる。それ以降の \<single-line-else-clause\> は、すでに対になっている \<if-then-else-statement\> の前にある最初の \<single-line-if-statement\> と対になる。
- \<statement-label\> は \<list-or-label\> 要素の最初の要素として存在し、\<statement-label\> は同じ \<statement-label\> を含む \<goto-statement\> に置き換えられたかのような効果を持つ。この \<goto-statement\> は、\<statement-label-list\> の \<line-number-label\> の代わりとなる。

実行時セマンティクス

- \<single-line-if-statement\> はその \<boolean-expression\> を評価し、もし式のデータ値が True であれば、キーワード `Then` に続く \<list-or-label\> 要素を実行する。もし、式のデータ値が False であれば、キーワード `Else` に続く \<list-or-label\> を実行する。
- \<list-or-label\> は、その構成要素である \<same-line-statement\> を順に実行し、最後に含まれる \<statement\> が実行されるか、実行されたステートメントが明示的に \<list-or-label\> の外に移るまで実行される。

#### 5.4.2.10 Select Case ステートメント

\<select-case-statement\> は、候補の中からどの \<statement-block\> を実行するかを決定する。
...
- \<select-case-statement\> では、\<select-expression\> が直ちに評価され、その後の \<case-clause\> と \<case-else-clause\> の評価で使用されます。
- 各 \<case-clause\> に対して、含まれる各 \<range-clause\> が定義された順序で評価される。\<range-clause\> が \<select-expression\> にマッチした場合、\<case-clause\> に含まれる \<statement-block\> が実行される。\<statement-block\> が実行されると、\<select-case-statement\> の実行は直ちに終了する（後続の \<case-clause\> は評価されない）。
    - \<range-clause\> が \<expression\> の場合、\<expression\> が評価され、その結果が \<select-expression\> の値と比較される。もしそれらが等しければ、\<range-clause\> は \<select-expression\> にマッチしたとみなされる。\<case-clause\> 内のそれ以降の \<range-clause\> は評価されない。
    - \<range-clause\> がキーワード `Is` または \<comparison-operator\> で始まっている場合、"\<select-expression\> \<comparison-operator\> \<expression\>" という式が評価される。この式の評価が True を返す場合、\<range-clause\> は \<select-expression\> にマッチするとみなされる。\<case-clause\> 内のそれ以降の \<range-clause\> は評価されない。
    - \<range-clause\> が \<start-value\> と \<end-value\> を持つ場合、式 "((\<select-expression\>) >= (\<start-value\>))" と式 "((\<select-expression\>) \<= (<end-value\>))" が評価される。この式の評価で True が返された場合、\<range-clause\> は \<select-expression\> にマッチしたとみなされる。\<case-clause\> の中のそれ以降の \<range-clause\> は評価されない。
- 各 \<case-clause\> 内の \<range-clause\> を評価した結果がマッチしない場合は \<case-else-clause\> 内の \<statement-block\> が実行される。\<select-expression\> が `Null` の場合、\<case-else-clause\> 内の \<statement-block\> のみが実行される。
- \<select-case-statement\> の外で定義された \<goto-statement\> が \<statement-block\> 内の \<statement\> を実行させた場合、\<select-expression\>, \<case-clause\>, \<range-clause\> のいずれも評価されない。\<goto-statement\> は、\<statement-block\> から脱出することもできる。後の \<goto-statement\> によって再び \<statement-block\> に入った場合、\<select-case-statement\> 内の \<statement-block\> の実行規則で指定された動作となる。

#### 5.4.2.11 Stop ステートメント
...
- \<stop-statement\> は、VBA プログラムの実行を実装定義の方法で一時停止させる。実行を再開できるかどうかは実装依存である。
- 実装定義された外部からの介入を受ける可能性があるが、実行が再開されてもすべての変数はその状態を維持する。

#### 5.4.2.12 GoTo ステートメント
...
- \<goto-statement\> を含むプロシージャは、\<goto-statement\> で定義された \<statement-label\> と同じ \<statement-label-definition\> を正確に 1 つ<ins>含まなければならない</ins>。

実行時セマンティクス

- \<goto-statement\> を指定すると、\<statement-label-definition\> の直後の \<statement\> に実行を分岐させる。
- \<statement-label\> が構文的にそれを囲むプロシージャ宣言の \<end-label\> と同じ場合、現在の \<procedure-body\> の実行は \<procedure-body\> 要素の \<statement-block\> の末尾に到達したかのように即座に終了する。
